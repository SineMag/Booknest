...
export const putIntoAdminQuery = (id: number, user: Admin) => sql`
  UPDATE admins SET firstName=${user.firstName} , lastName=${user.lastName}, emailAddress=${user.emailAddress}, physicalAddress=${user.physicalAddress}, phoneNumber=${user.phoneNumber}, profilePicUrl=${user.profilePicUrl}, password=${user.password} WHERE id=${id} RETURNING *;`;

export const insertAdminQuery = (user: Admin) => sql`
  INSERT INTO admins (firstName, lastName, emailAddress, physicalAddress, phoneNumber, profilePicUrl, password)
...
export const loginAdminQuery = (credentials: LoginCredentials) => sql`
  SELECT * FROM admins WHERE emailAddress=${credentials.emailAddress} AND password=${credentials.password} RETURNING *;`;
